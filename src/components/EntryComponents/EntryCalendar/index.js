...
const mapStateToProps = ({ Calendar: { activeDate, view }, Entries: { items } }) => ({
  entries: items,
  activeDate,
  view,
})
...
const EntryCalendar = ({ entries, activeDate, view, SetCalendar, GetUserEntriesByDate }) => {
  useEffect(() => {
    const payload = {
  ...
  const entriesWithinView = useMemo(
    () =>
      entries
        ...
        .sort((a, b) => new Date(a.date_created_by_author) - new Date(b.date_created_by_author)),
    [activeDate, entries],
  )
  ...
  const handleDateChange = ({ activeStartDate, view }) => {
    ...
    const shouldGetUserEntries =
      new Date(previousActiveStartDate.current).getMonth() !== activeDate.getMonth()
  ...
  const handleOnChange = activeStartDate => handleDateChange({ activeStartDate, view }, false)
  ...
  return (
    <Container fluid className='EntryCalendar Container'>
      <Row>
        <Col className='EventList' xs={{ size: 12, order: 2 }} md={{ size: 3, order: 1 }}>
          <div className='EventListHeader Center'>
            <Button color='inherit' className='TodayButton mr-1' onClick={handleTodayClick}>
              <i className='fas fa-calendar-day NavBarImage NavBarLink' />
            </Button>
            <Moment format='MMM D'>{activeDate}</Moment>
            <Button color='inherit' className='TodayButton ml-1' onClick={handleNewEntryClick}>
              <i className='fas fa-feather-alt NavBarImage NavBarLink' />
            </Button>
          </div>
          <EntryList activeDate={activeDate} entriesWithinView={entriesWithinView} />
        </Col>
        <Col xs={{ size: 12, order: 1 }} md={{ size: 9, order: 2 }} className='p-0'>
