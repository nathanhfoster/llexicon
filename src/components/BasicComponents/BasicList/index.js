...
export class BasicList extends PureComponent {
  ...
  static propTypes = {
    ...
    scrollToItem: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    const { scrollToItem: prevScrollToItem } = prevProps
    const { scrollToItem: nextScrollToItem } = this.props
    if (prevScrollToItem !== nextScrollToItem && nextScrollToItem !== -1) {
      this.listRef.current.scrollToItem(nextScrollToItem, 'center')
    }
  }
  ...
  renderList = ({ data, index, style, isScrolling }) => {
    ...
    const { onListItemClickCallback, listItemHoverable, itemSize, listItemStyles } = this.props
      ...
      >
        {typeof value === 'object' ? (
          ...
          <Fragment>
            ...
            {otherValue && <span className='Overflow OtherValue'>{otherValue}</span>}
          </Fragment>
        )}
      </div>
    )
  ...
  render() {
    const { className, itemSize, listPosition, list, height, width, layout, render } = this.props
    const styles = {
