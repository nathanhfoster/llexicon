...
const useGeolocationEvents = (options = geolocationStandardOptions) => {
  ...
  const [onChangeRef, setOnChangeRef] = useCreateHandlerSetter()
  const [onErrorRef, setOnErrorRef] = useCreateHandlerSetter()
  ...
  return !isSupported
    ? createCbSetterErrorProxy('The Geolocation API is not supported')
    : Object.freeze({
        isSupported,
