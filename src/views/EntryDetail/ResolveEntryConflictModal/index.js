...
const ResolveEntryConflictModal = ({
  ...
  useEffect(() => {
    const userDidNotUpdateEntryInRedux = previousEntry._lastUpdated === entry._lastUpdated
    const hasDifferentProps = findDifferentProps(entryFromServer, entry).length > 0
  ...
  const handleSave = useCallback(async () => {
    ...
    entryToUpdate.id && SetEntry(entryToUpdate, updateDate)
      ...
      {entry && entryFromServer && (
        <Container className='Container'>
          <Row>
            ...
            <Col xs={{ size: 12, order: 2 }} md={{ size: 12, order: 3 }} className='p-2 md-p-1'>
...
export default reduxConnect(mapStateToProps, mapDispatchToProps)(ResolveEntryConflictModal)
